...
- (void)handleExtensionData:(unsigned char *)dp length:(size_t)dataLength {
		...
		if (expType == WiiNunchuk) {
			...
			*/
			...
			cButtonData = (UInt16)([self decrypt:dp[startByte + 4]] << 8) 
							+ [self decrypt:dp[startByte + 5]];
			cButtonData = ~cButtonData;	// bit reverse
			...
			cStickX2 = (([self decrypt:dp[startByte]] & 0xC0) >> 3 |
						([self decrypt:dp[startByte +1]] & 0xC0) >> 5 |
						([self decrypt:dp[startByte +2]] & 0x80) >> 7) & 0x1F; 
			cStickY2 =  [self decrypt:dp[startByte +2]] & 0x1F;
			
			cAnalogL = (( [self decrypt:dp[startByte +2]] & 0x60) >> 2 | ( [self decrypt:dp[startByte +3]] & 0xE0) >> 5) & 0x1F ;
	...
	if (isExpansionPortEnabled) {
		if (expType == WiiNunchuk) {
			...
			//NSLog(@"cStickX2 = %i cStickY2 = %i cAnalogL = %i cAnalogR = %i",cStickX2,cStickY2,cAnalogL,cAnalogR);
		}			
	}
} // handleExtensionData
...
//	NSLog(@"IR Data %00x,%00x  %00x,%00x", irData[0].x, irData[0].y, irData[1].x, irData[1].y);
	...
	if (irData[0].s < 0x0F && irData[1].s < 0x0F) {
		...
		if(angle > M_PI*-1.0 && angle < M_PI) irTracking = TRUE;
	...
	if ([_delegate respondsToSelector:@selector(irPointMovedX:Y:ANGLE:tracking:wiiRemote:)]){
		[_delegate irPointMovedX:ox Y:oy ANGLE:angle tracking:irTracking wiiRemote:self];
	}
...
- (void)sendWiiRemoteButtonEvent:(UInt16)data{
	[_delegate reportRemoteButtonData:data	wiiRemote:self];
}

- (void)sendWiiNunchukButtonEvent:(UInt16)data{
	[_delegate reportNunchuckButtonData:data	wiiRemote:self];

}

- (void)sendWiiClassicControllerButtonEvent:(UInt16)data{
	[_delegate reportClassicControllerButtonData:data	wiiRemote:self];
